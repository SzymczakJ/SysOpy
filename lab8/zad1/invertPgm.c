...
void *thread_job(void *data) {
    ...
    if (job->jobType == NUMBERS) {
        int lowerIntervalStart, lowerIntervalEnd, higherIntervalStart, higherIntervalEnd;
        lowerIntervalStart = 128 / job->nJobs * job->myIndex;
        lowerIntervalEnd = 128 / job->nJobs * (job->myIndex + 1) - 1;
        higherIntervalEnd = 255 - lowerIntervalStart;
        higherIntervalStart = 255 - lowerIntervalEnd;

        for (int i = 0; i < job->height; i++) {
            for (int j = 0; j < job->width; j++) {
                uint value = job->inData[i][j];
                if ((value >= lowerIntervalStart && value <= lowerIntervalEnd) || (value >= higherIntervalStart && value <= higherIntervalEnd)) {
                    job->outData[i][j] = 255 - value;
                }
            }
        }
    }
    else {
        int startColumn = job->myIndex / job->nJobs * job->width;
        int endCol;
    ...
    job->rval = 1;
    pthread_exit(&(job->rval));
}


int main(int argc, char **argv) {
    int nThreads, partitionType;
    char *inputFileName;
    char *outputFileName;
    if(argc < 5){
    ...
    uint **image = loadImage(input, &imageHeight, &imageWidth);
    if(image == NULL) {
        puts("invalid file format");
        return 0;
    }

    uint **newImage = createImage(imageHeight, imageWidth);
