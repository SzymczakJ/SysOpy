...
execData * commandsToExecutions(char ** commands, size_t nCommands){
...
char ** pipeSplit(char * text, size_t * rL, bool strip){
    char * ptr = strtok(text, "|");
    char ** result = NULL;
    size_t numberOfWords = 0;
    while(ptr != NULL){
        result = realloc(result, sizeof(char*)*(numberOfWords + 1));
        if(strip){
            char * interm = strdup(ptr);
            result[numberOfWords++] = readWord(interm);
            free(interm);
        }
        else
            result[numberOfWords++] = strdup(ptr);
        ptr = strtok(NULL, "|");
    }
    *rL = numberOfWords;
    return result;
    ...
    for(size_t lineIndex = 0; lineIndex < lineCount; lineIndex++){
        ...
        for(size_t i = offset; i < fileSize; i++){
            ...
            if(fileContent[i] == '#' || fileContent[i] == '\n' || fileContent[i] == 0x0){
                if(foundNonWhite){
                    validLines = realloc(validLines, sizeof(char*)* ++validLinesNumber);
                    validLines[validLinesNumber - 1] = strndup(fileContent + offset, i-offset);
                    eqPositions = realloc(eqPositions, sizeof(int)* validLinesNumber);
                    eqPositions[validLinesNumber - 1] = eqPosition;
                }
                while(fileContent[i] != '\n' && fileContent[i] != 0x0)
                    i++;
                offset = i+1;
...
appData fileParse(char * fileContent, size_t fileSize){
    ...
    size_t validLinesNumber = readLines(&validLines, &eqPositions, lineCount, fileContent, fileSize);

    for(size_t i = 0; i < validLinesNumber; i++){
        if(eqPositions[i] == -1){
            size_t numberOfWords;
            char ** varNames = pipeSplit(validLines[i], &numberOfWords, true);
            ...
            result.commandList[result.nCommands-1].nCommands = numberOfWords;
        }
        else{
            validLines[i][eqPositions[i]] = 0x0;
            char * name = readWord(validLines[i]);
            size_t nCommands;
